...
const Organisations = () => {
  ...
  const renderFile = (fileUrl) => {
    ...
    switch (fileType) {
      case "image":
        return <img src={fileUrl} alt="Логотип организации"/>;
      default:
      ...
      <div className="team">
        {organisations.map((organisation) => (
          <div key={organisation.id} className="teamMember">
            <Link to={organisation.link}> {renderFile(`${process.env.REACT_APP_API_URL}/uploads/${encodeURIComponent(organisation.image)}`)} {/* <img src={organisation.image}/>*/}</Link> 
          </div>
        ))}
      </div>
      <Footer />
